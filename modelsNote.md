...
**Decision Tree**를 사용하여 **입양 여부**를 예측을 목표로 모델을 학습하였습니다. 
이 문서는 모델링 작업 과정을 설명합니다. 모델링 과정에서 사용된 라이브러리 도구와 데이터셋, 성능 지표를 정리하였습니다.
...
### 설명
사전 학습된 KoBERT 모델을 사용하여 텍스트 데이터에 대해 **감성 분석**을 수행하여 데이터의 긍정(Positive), 부정(Negative), 중립(Neutral) 발생 개수를 추출
