...
def preprocess_color(dataframe):
    ...
    dataframe['색상'] = dataframe['색상'].apply(
        lambda x: 'Mix' if re.fullmatch(r'(혼합색|혼합|믹스)', x) else re.sub(r'(혼합|믹스)', '', x))

    # 색상 대체 딕셔너리 정의 (순서 중요)
    color_replacements = {
        '흰암|암': 'Ivory',
        'white|화이트|하얀|하양|연흰|흐니|백구|밀크|희|흰|백|벡|힌|휜|흔|힁|힄|횐|읜': 'White',
        '블루탄': 'Blue_tan',
        ...
        'ivory|dkdlqhfl|cream|아이보리|아아보리|이이보리|아리보리|아이?리|앙보리|연 노랑|연노랑|연노란|크림림|오트밀|누룽지|크림크|크리색|크램색|연회|연노|크림|상아|레몬|진미|살구|살|미': 'Ivory',

        '적황|짙은 황|짙은황': 'Brown',

        '여튼브라운|연한 황토|연한 갈|옅은 갈|옅은 황|연한 황|밝은 갈|흐린 갈|여튼갈|연한황|옅은황|밝은갈|옅은갈|엷은갈|연한갈|진한갈|흐린갈|연 갈|연,갈': 'Beige',
        'Apricot|apricot|gold|배아자샥|베이지|배아쟈|옐로우|진노랑|오랜지|연갈|담황|인절|치즈|황생|황토|황구|황금|화운|골든|골드|연황|진황|진베|누런|누렁|노랑': 'Beige',
